...
// 디버그 로그를 자세히 남기는 fetch
async function fetchJson<T>(path: string, init?: RequestInit, tag?: string): Promise<T> {
  ...
  try {
    ...
    // 개발 중에만 남겨도 됨
    console.log(`[${tag ?? path}] status:`, res.status, res.statusText);
    if (raw) console.log(`[${tag ?? path}] raw:`, raw);
    ...
    // 서버가 ApiEnvelope로 안 내려오는 경우도 방어
    return (data as T) ?? ({} as T);
  } catch (err: any) {
    // 네트워크 오류 메시지 보강
    const m = err?.message || "Network error";
    throw new Error(`[${tag ?? path}] ${m}`);
  }
}
