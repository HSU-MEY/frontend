...
import { signupApi } from '@/api/auth.service'; // 공통 API 사용
...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // (선택) selectedLanguage는 현재 서버 스펙에 없으므로 저장/전송 생략
      Alert.alert('회원가입 완료!', '이제 로그인해주세요.');
      router.replace('/account/login');
    } catch (error: any) {
      console.error('회원가입 에러:', error);
  ...
  return (
    <Container>
      ...
      <Input placeholder="이름" value={nickname} onChangeText={setNickname} />

      <Input placeholder="이메일" value={email} onChangeText={setEmail} autoCapitalize="none" />
      ...
      <Button title={submitting ? '가입 중…' : '회원가입'} onPress={handleRegister} disabled={submitting} />
